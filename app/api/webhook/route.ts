...
async function createAndFinalizeInvoice(
  ...
  try {
    ...
    // Validate currency
    const currency = (orderData.currency || "KSh").toLowerCase();
    if (!["usd", "eur", "gbp", "cad", "aud", "kes"].includes(currency)) {
      console.warn(`Unsupported currency: ${currency}, defaulting to KSh`);
    }
