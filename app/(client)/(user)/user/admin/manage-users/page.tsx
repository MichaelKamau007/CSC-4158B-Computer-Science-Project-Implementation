...
export default function AdminUserManagement() {
  ...
  useEffect(() => {
    if (isLoaded && !user) {
      router.push("/sign-in?redirectTo=/user/admin/manage-users");
    } else if (isLoaded && user) {
      // Check if user is admin (you can adjust this logic based on your admin check)
      const userEmail = user.emailAddresses?.[0]?.emailAddress;
      if (userEmail !== "dev.reactbd@gmail.com") {
        router.push("/user/dashboard"); // Redirect non-admin users to dashboard
